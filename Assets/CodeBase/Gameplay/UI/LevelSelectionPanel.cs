...
namespace SpaceShooter
{
    public class LevelSelectionPanel : MonoBehaviour
    {
        ...
        private void Start()
        {
            LevelProperties[] levelProperties = LevelSequencesController.Instance.LevelSequences.LevelsPropetries;
            for (int i = 0; i < levelProperties.Length; i++)
            {
                LevelSelectionButton levelSelectionButton = Instantiate(m_LevelButtonPrefab);
                levelSelectionButton.SetLevelPropetries(levelProperties[i]);
